...
- 使用 Emoji 图标增强识别度
- 按角色分组着色：
  - 🟢 绿色 (4): 用户侧（管理员、前端、用户钱包）
  - 🔵 蓝色 (5): 合约/系统侧
  - 🟡 黄色 (3): 外部服务
  - 🟣 紫色 (6): VRF/Oracle/异步服务

**2. 左侧阶段标签（垂直排列）**

- 使用大型半透明容器（group 节点）作为背景分组
- 每个阶段使用不同颜色标识

**3. 中央流程区（垂直流向）**

- 动作卡片：白色背景，左上角蓝底白字数字编号 `[1]-[N]`
- 条件分支容器：
  - 🔴 红色 (1)：路径 A
  - 🟠 橙色 (2)：路径 B
- 平滑连线：带标签的箭头连接
...
2. **色彩层次**：
   ...
   - 阶段标签使用中等饱和度
   - 注释说明使用浅色背景
3. **可读性优先**：深色文字配浅色背景，浅色文字配深色背景
4. **视觉引导**：使用鲜明颜色引导用户视线沿着流程走
