...
> **背景**: Codex 探索代码库时可能读取 node_modules 等大目录，导致 token 消耗激增（观察到 500 万+ tokens）

### 默认约束（强制添加到每个 Codex prompt 开头）

```
⚠️ 约束（必须遵守）:
- 禁止读取 node_modules 目录
- 禁止读取 target/debug、target/release、dist、build 目录
- 禁止探索超过 2 层的依赖关系
- 单次任务读取文件不超过 20 个
```
...
| 策略 | 说明 | 示例 |
|------|------|------|
| **内联参考代码** | 在 prompt 中直接给出关键代码片段 | 不要说「参考 sdk.ts」，而是直接贴出相关函数 |
| **限定文件范围** | 明确列出需要读取的文件路径 | `只读: src/utils/helper.ts 第 10-50 行` |
| **给出代码模板** | 直接给出实现骨架 | 提供函数签名和关键 import |
| **禁止探索** | 明确禁止读取不需要的目录 | `禁止: node_modules, target, dist` |
...
## 参考文件（只读这些）
{related_files_with_line_numbers}
...
# 返回 output_file 路径
# 使用 tail -f <output_file> 实时查看
```

### Token 检查点

执行完成后检查 Codex 日志中的 `usage` 字段：

| input_tokens | 状态 | 行动 |
|--------------|------|------|
| < 500K | ✅ 正常 | 继续 |
| 500K - 1M | ⚠️ 警告 | 检查是否读取了不必要的文件 |
| 1M - 5M | 🔴 异常 | 分析原因，优化 prompt |
| > 5M | 🚨 严重 | 任务可能陷入循环，立即终止 |

### 诊断命令

检查 Codex 输出日志的 token 使用：

```bash
# 提取最后一行的 usage 信息
tail -1 codex-outputs/batch-X/*.log | grep -o '"input_tokens":[0-9]*'
```
