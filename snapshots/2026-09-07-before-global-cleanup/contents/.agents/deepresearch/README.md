...
When research runs inside an Obsidian vault, reader-facing reports are saved to `300_Resources/Res_调研/[Topic]_Research_[Date]/` within that vault. Outside a vault, reports are saved to `~/Documents/[Topic]_Research_[Date]/`.

The resource folder contains the Markdown source report, HTML, PDF, and a concise README. Source registries, evidence ledgers, claim ledgers, manifests, and temporary render files remain outside the vault because they are machine-oriented and regenerable.
