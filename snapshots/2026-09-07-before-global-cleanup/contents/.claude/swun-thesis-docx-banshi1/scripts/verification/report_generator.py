...
from modules.caption_profile import (
    ...
    normalize_caption_profile,
    paragraph_signature,
    profile_signature,
    resolve_caption_profile_docx,
)
...
def _resolve_caption_profile_docx(docx_path: str | None) -> Path:
    root = (
        Path(docx_path).expanduser().resolve().parent
        if docx_path else None
    )
    return resolve_caption_profile_docx(root)
...
def _collect_caption_paragraph(
    ...
    if body is None:
        return None
    # 必须是“图/表 + 章-序号”题注格式，避免误匹配以图/表开头的普通标题或正文
    caption_re = re.compile(rf"^{prefix}\s*\d+[\-－.．]\d+")
