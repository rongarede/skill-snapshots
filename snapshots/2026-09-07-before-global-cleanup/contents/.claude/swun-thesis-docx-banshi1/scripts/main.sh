...
# 解析顺序：env > 论文目录参考论文 > skill 内置瘦身模板（assets/）
ASSET_REFERENCE_DOCX="$SCRIPT_DIR/../assets/swun_banshi1_reference.docx"
REFERENCE_DOCX="${SWUN_REFERENCE_DOCX:-}"
if [[ -z "$REFERENCE_DOCX" ]]; then
  if [[ -f "$THESIS_DIR/reference_thesis.docx" ]]; then
    REFERENCE_DOCX="$THESIS_DIR/reference_thesis.docx"
  else
    REFERENCE_DOCX="$ASSET_REFERENCE_DOCX"
  fi
fi
NORMALIZED_DOCX="$THESIS_DIR/.reference_normalized.docx"
MAPPING_JSON="$SCRIPT_DIR/style_id_mapping.json"

if [[ ! -f "$REFERENCE_DOCX" ]]; then
  echo "error: reference template not found: $REFERENCE_DOCX" >&2
  exit 1
fi
if [[ ! -f "$MAPPING_JSON" ]]; then
  echo "error: style mapping not found: $MAPPING_JSON" >&2
  exit 1
fi

echo "[1/6] Normalizing reference template style IDs... ($REFERENCE_DOCX)"
...
export SWUN_CAPTION_PROFILE_DOCX="${SWUN_CAPTION_PROFILE_DOCX:-$REFERENCE_DOCX}"
