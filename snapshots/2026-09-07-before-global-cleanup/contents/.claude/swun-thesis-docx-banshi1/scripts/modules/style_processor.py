...
<w:abstractNum w:abstractNumId="88880" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  ...
  <w:lvl w:ilvl="0">
    ...
    <w:lvlJc w:val="left"/>
    <w:suff w:val="space"/>
    <w:pPr><w:ind w:left="0" w:firstLine="0" w:hanging="0"/></w:pPr>
  </w:lvl>
  <w:lvl w:ilvl="1">
    <w:start w:val="1"/>
    <w:numFmt w:val="decimal"/>
...
def ensure_indent_for_body_paragraphs(
    ...
    # styleId "9" 是 Body Text (该模板的本地命名), 行为与 Normal 一致.
    body_styles = {"BodyText", "FirstParagraph", "Compact", "a", "9"}
    # Heading2 (二级子标题) must be flush-left; override numbering indent.
    # Heading3 保留首行缩进 (与参考文档一致), 不在此处理.
    flush_left_styles = {"Heading2", "2"}
    ...
    # 阶段说明类段落识别: 以"X 阶段:" / "X 阶段：" 开头的段落不缩进
    # (e.g. Prepare 阶段, Pre-commit 阶段, Commit 阶段, Decide 阶段, ... )
    import re as _re
    stage_pattern = _re.compile(r'^[A-Za-z][\w-]*\s*阶段[：:]')
    ...
    for child in list(body):
        ...
        if style in body_styles:
            ...
            if prev_sig is not None and is_display_math_para(prev_sig):
                ...
                continue
            # 阶段说明段落: 顶格无缩进
            txt = _p_text(ns, child).strip()
            if stage_pattern.match(txt):
                _clear_para_first_indent(ns, child)
                prev_sig = child
                continue
            if ind is None:
            ...
            if ind is None:
                ind = ET.SubElement(pPr, w_ind)
            # Override numbering indent: flush left, no hanging, no first-line.
            ind.set(w_left, "0")
            ind.set(_qn(ns, "w", "leftChars"), "0")
...
def align_styles_to_reference(styles_xml: bytes) -> bytes:
    ...
    w_rFonts = _qn(sns, "w", "rFonts")
    w_val = _qn(sns, "w", "val")
    w_name = _qn(sns, "w", "name")
    w_basedOn = _qn(sns, "w", "basedOn")
    ...
    "afterLines",
     "afterAutospacing"):
        ...
        # 1b) Body Text style (styleId='9', name='Body Text', basedOn='a').
        # 模板原值 spacing.after=120 比 Normal 多 6pt 段后空白, 导致与 Normal
        # 段落混排时段间距不一致. 强制清零 spacing 让其完全继承 Normal.
        elif sid_norm == "9" or name_norm in ("bodytext", "bodytextindent"):
            pPr = _ensure(st, w_pPr)
            ...
            if ind is not None:
                pPr.remove(ind)
            updated.append(f"BodyText(styleId={sid or 'unknown'})")

        # 2) Heading 3 style (styleId='Heading3', style name='heading 3').
        # 宋体不加粗 12pt(小四), 段前 6pt 段后 3pt, 1.5 倍行距,
        # 首行缩进 2 字符 (firstLine=482, 与参考文档一致).
        elif sid_norm in {"heading3", "3"} or name_norm == "heading3":
            pPr = _ensure(st, w_pPr)
            ind = _ensure(pPr, w_ind)
            ind.set(_qn(sns, "w", "firstLine"), "482")
            ind.set(_qn(sns, "w", "firstLineChars"), "200")
            ind.set(_qn(sns, "w", "left"), "0")
            ind.set(_qn(sns, "w", "leftChars"), "0")
            ...
            spacing = _ensure(pPr, w_spacing)
            spacing.set(_qn(sns, "w", "before"), "120")
            spacing.set(_qn(sns, "w", "after"), "60")
            spacing.set(_qn(sns, "w", "line"), "360")
            spacing.set(_qn(sns, "w", "lineRule"), "auto")

            rPr = _ensure(st, w_rPr)
            rfonts = _ensure(rPr, w_rFonts)
            rfonts.set(_qn(sns, "w", "ascii"), "Times New Roman")
            rfonts.set(_qn(sns, "w", "hAnsi"), "Times New Roman")
            rfonts.set(_qn(sns, "w", "eastAsia"), "宋体")
            rfonts.set(_qn(sns, "w", "cs"), "Times New Roman")
            for attr in ("asciiTheme", "hAnsiTheme", "eastAsiaTheme", "cstheme"):
                q_attr = _qn(sns, "w", attr)
                if q_attr in rfonts.attrib:
                    del rfonts.attrib[q_attr]
            sz = _ensure(rPr, w_sz)
            sz.set(w_val, "24")  # 12pt 小四
            szCs = _ensure(rPr, w_szCs)
            szCs.set(w_val, "24")
            # 移除加粗 (heading 3 不加粗)
        ...
        # 2b) Heading 2 style (styleId='Heading2', style name='heading 2').
        # 宋体不加粗 14pt(四号), 段前 12pt 段后 6pt, 1.5 倍行距, 顶格无首行缩进.
        elif sid_norm in {"heading2", "2"} or name_norm == "heading2":
            pPr = _ensure(st, w_pPr)
            outline = _ensure(pPr, w_outlineLvl)
            outline.set(w_val, "1")

            ind = _ensure(pPr, w_ind)
            ind.set(_qn(sns, "w", "firstLine"), "0")
            ind.set(_qn(sns, "w", "left"), "0")
            for attr in ("firstLineChars", "leftChars", "hanging", "hangingChars"):
                q_attr = _qn(sns, "w", attr)
                if q_attr in ind.attrib:
                    del ind.attrib[q_attr]

            spacing = _ensure(pPr, w_spacing)
            spacing.set(_qn(sns, "w", "before"), "240")
            spacing.set(_qn(sns, "w", "after"), "120")
            spacing.set(_qn(sns, "w", "line"), "360")
            spacing.set(_qn(sns, "w", "lineRule"), "auto")
            ...
            updated.append(f"Heading2(styleId={sid or 'unknown'})")

        # 3) Heading 1 style (styleId='Heading1', style name='heading 1').
        # 黑体加粗 15pt(小三), 居中, 章前另起一页 (pageBreakBefore 由别处注入).
            ...
            rfonts.set(_qn(sns, "w", "ascii"), "Times New Roman")
            rfonts.set(_qn(sns, "w", "hAnsi"), "Times New Roman")
            rfonts.set(_qn(sns, "w", "eastAsia"), "黑体")
            rfonts.set(_qn(sns, "w", "cs"), "Times New Roman")
            for attr in ("asciiTheme", "hAnsiTheme", "eastAsiaTheme", "cstheme"):
                q_attr = _qn(sns, "w", attr)
                if q_attr in rfonts.attrib:
                    del rfonts.attrib[q_attr]
            sz = _ensure(rPr, w_sz)
            sz.set(w_val, "30")  # 15pt 小三
            szCs = _ensure(rPr, w_szCs)
            szCs.set(w_val, "30")
            if rPr.find(w_b) is None:
                ET.SubElement(rPr, w_b)
            if rPr.find(w_bCs) is None:
                ET.SubElement(rPr, w_bCs)

            updated.append(f"Heading1(styleId={sid or 'unknown'})")

        # 4) toc 1/2/3: 字号 12pt(小四), 缩进 0/2字符/4字符, 1.5 倍行距.
        elif name_norm in {"toc1", "toc2", "toc3"}:
            level = int(name_norm[-1])  # 1/2/3
            pPr = _ensure(st, w_pPr)

            ind = _ensure(pPr, w_ind)
            for attr in ("hanging", "hangingChars", "firstLine", "firstLineChars"):
                q_attr = _qn(sns, "w", attr)
                if q_attr in ind.attrib:
                    del ind.attrib[q_attr]
            indent_chars = (level - 1) * 2  # toc1=0, toc2=2, toc3=4 (字符)
            ind.set(_qn(sns, "w", "leftChars"), str(indent_chars * 100))
            ind.set(_qn(sns, "w", "left"), str(indent_chars * 240))

            spacing = _ensure(pPr, w_spacing)
            spacing.set(_qn(sns, "w", "before"), "0")
            spacing.set(_qn(sns, "w", "after"), "0")
            spacing.set(_qn(sns, "w", "line"), "360")
            spacing.set(_qn(sns, "w", "lineRule"), "auto")
            ...
            updated.append(f"{name_norm}(styleId={sid or 'unknown'})")
