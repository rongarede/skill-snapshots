...
# skill 内置瘦身参考模板（scripts/make_slim_template.py 生成）
ASSET_REFERENCE_DOCX = (
    Path(__file__).resolve().parents[2] / "assets"
    / "swun_banshi1_reference.docx"
)
_LEGACY_ROOT = Path("/Users/bit/LaTeX/SWUN_Thesis")


def resolve_caption_profile_docx(root: Path | None = None) -> Path:
    """题注样例来源解析：env > 论文目录参考论文 > skill 内置模板。"""
    env = os.environ.get("SWUN_CAPTION_PROFILE_DOCX")
    if env:
        return Path(env).expanduser()
    local = Path(root or _LEGACY_ROOT) / "reference_thesis.docx"
    return local if local.exists() else ASSET_REFERENCE_DOCX
...
def normalize_caption_profile(profile: CaptionFormatProfile) -> None:
    """规范化图表标题: 中英文混排时统一字体字号缩进, 保证视觉一致.

    - 字号 9pt(小五号), val=18
    - 中文宋体, 英文 Times New Roman, 加粗
    - 居中, 无首行/左缩进
    - 段前 6pt, 段后 6pt, 单倍行距
    """
    w_val = _qn("val")

    pp_kept: list[ET.Element] = []
    for child in profile.paragraph_props:
        local = child.tag.split("}", 1)[-1]
        if local in {"ind", "spacing", "jc"}:
            continue
        pp_kept.append(child)
    jc = ET.Element(_qn("jc"))
    jc.set(w_val, "center")
    pp_kept.append(jc)
    spacing = ET.Element(_qn("spacing"))
    spacing.set(_qn("before"), "120")
    spacing.set(_qn("after"), "120")
    spacing.set(_qn("line"), "276")
    spacing.set(_qn("lineRule"), "auto")
    pp_kept.append(spacing)
    profile.paragraph_props = pp_kept

    rp_kept: list[ET.Element] = []
    for child in profile.run_props:
        local = child.tag.split("}", 1)[-1]
        if local in {"rFonts", "sz", "szCs", "b", "bCs"}:
            continue
        rp_kept.append(child)
    rfonts = ET.Element(_qn("rFonts"))
    rfonts.set(_qn("ascii"), "Times New Roman")
    rfonts.set(_qn("hAnsi"), "Times New Roman")
    rfonts.set(_qn("cs"), "Times New Roman")
    rfonts.set(_qn("eastAsia"), "宋体")
    rp_kept.append(rfonts)
    sz = ET.Element(_qn("sz"))
    sz.set(w_val, "18")
    rp_kept.append(sz)
    szCs = ET.Element(_qn("szCs"))
    szCs.set(w_val, "18")
    rp_kept.append(szCs)
    rp_kept.append(ET.Element(_qn("b")))
    rp_kept.append(ET.Element(_qn("bCs")))
    profile.run_props = rp_kept
