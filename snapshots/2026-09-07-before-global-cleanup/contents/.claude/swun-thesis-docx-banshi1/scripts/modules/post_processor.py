...
try:
    from modules.caption_profile import (
        ...
        ASSET_REFERENCE_DOCX as _ASSET_REFERENCE_DOCX,
        resolve_caption_profile_docx as _resolve_caption_profile_docx,
    )
except ModuleNotFoundError:
...
try:
    from modules.figure_table_handler import (
        apply_data_table_pagination as _apply_data_table_pagination,
...
try:
    from modules.equation_handler import (
        ...
        normalize_math_fonts as _normalize_math_fonts,
    )
except ModuleNotFoundError:
...
def _resolve_template_docx(root: Path) -> Path:
    """模板解析：env > 论文目录已归一化模板 > skill 内置瘦身模板。"""
    env = os.environ.get("SWUN_TEMPLATE_DOCX")
    if env:
        return Path(env).expanduser()
    local = root / ".reference_normalized.docx"
    return local if local.exists() else _ASSET_REFERENCE_DOCX


TEMPLATE_DOCX = _resolve_template_docx(ROOT)
CAPTION_PROFILE_DOCX = _resolve_caption_profile_docx(ROOT)
...
def _postprocess_docx(
    ...
    doc_ns, body, caption_meta, caption_profiles)
        ...
        # 数据表分页规则: 行内 cantSplit, 首行 tblHeader 重复
        _n_data_tbl = _apply_data_table_pagination(doc_ns, body)
        if _n_data_tbl:
            print(f"  [pagination] Applied cantSplit + tblHeader to {_n_data_tbl} data table(s)")
        ...
        # 公式字体规范化（OMML math run 统一为 Times New Roman）
        _normalize_math_fonts(body, doc_ns)
...
def _resolve_paths(thesis_dir: Path) -> None:
    ...
    global TEMPLATE_DOCX, CAPTION_PROFILE_DOCX, _DEFAULT_CAPTION_PROFILES  # noqa: PLW0603
