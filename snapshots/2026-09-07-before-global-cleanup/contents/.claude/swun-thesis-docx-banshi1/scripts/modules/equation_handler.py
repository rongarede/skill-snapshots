...
def normalize_math_fonts(body: ET.Element, ns: dict[str, str], font_name: str = "Times New Roman") -> None:
    """将 OMML 公式内 math run（m:r）的字体设置为指定字体。

    遍历所有 m:oMath 和 m:oMathPara 下的 m:r 元素，
    在其 w:rPr/w:rFonts 上设置 w:ascii 和 w:hAnsi 为指定字体。
    """
    m_uri = None
    for k, v in ns.items():
        if k == "m":
    ...
    def _process_math_run(r: ET.Element) -> None:
        rPr = r.find(w_rPr)
        if rPr is None:
            rPr = ET.Element(w_rPr)
            r.insert(0, rPr)
        fonts = rPr.find(w_rFonts)
        if fonts is None:
            fonts = ET.SubElement(rPr, w_rFonts)
        fonts.set(w_ascii, font_name)
        fonts.set(w_hAnsi, font_name)

    for math_container in body.iter():
        if math_container.tag not in (m_oMath, m_oMathPara):
            continue
        for r in math_container.iter(m_r):
            _process_math_run(r)
