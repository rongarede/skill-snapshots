...
try:
    from modules.caption_profile import build_caption_paragraph, CaptionFormatProfile, extract_caption_profiles, normalize_caption_profile, resolve_caption_profile_docx
except ModuleNotFoundError:  # pragma: no cover - pytest imports via scripts.modules
    from scripts.modules.caption_profile import build_caption_paragraph, CaptionFormatProfile, extract_caption_profiles, normalize_caption_profile, resolve_caption_profile_docx
...
CAPTION_PROFILE_DOCX = resolve_caption_profile_docx()
...
def load_caption_profiles(
    ...
    for prof in profiles.values():
        normalize_caption_profile(prof)
    return profiles
...
def set_row_tbl_header(ns: dict[str, str], tr: ET.Element) -> None:
    """标记 tr 为表头, 跨页拆分时自动重复."""
    w_trPr = qn(ns, "w", "trPr")
    w_tblHeader = qn(ns, "w", "tblHeader")
    trPr = tr.find(w_trPr)
    ...
    if trPr.find(w_tblHeader) is None:
        ET.SubElement(trPr, w_tblHeader)


def apply_data_table_pagination(ns: dict[str, str], body: ET.Element) -> int:
    """对 body 中数据表 (非图片包装表) 应用合理的跨页规则:

    - 每行加 cantSplit, 防止单行被拆到两页
    - 第一行加 tblHeader, 跨页时表头自动重复
    ...
    for tbl in body.findall(w_tbl):
        if list(tbl.iter(w_drawing)):
            continue  # 跳过图片包装表 (wrap_figure_with_captions 已设过 cantSplit)
        rows = tbl.findall(w_tr)
        if not rows:
            continue
        for tr in rows:
            set_row_cant_split(ns, tr)
        set_row_tbl_header(ns, rows[0])
        n += 1
    return n
