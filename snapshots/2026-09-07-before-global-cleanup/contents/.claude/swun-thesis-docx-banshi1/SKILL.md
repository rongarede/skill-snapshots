...
- Reference template (resolution order):
  1. `SWUN_REFERENCE_DOCX` env var
  2. `{thesis_dir}/reference_thesis.docx` (per-thesis override)
  3. skill-bundled slim template `assets/swun_banshi1_reference.docx` (default, self-contained)
- The bundled slim template is generated by `scripts/make_slim_template.py` from the full reference thesis: it keeps styles/numbering/settings/headers/footers, the cover pages (incl. the first inline `sectPr` that pandoc reads for page geometry), and one sample figure/table caption paragraph for caption-profile extraction, while stripping all other body content and unreferenced media (7.4MB -> ~90KB)
...
# Optionally override reference template for the build script
export SWUN_REFERENCE_DOCX="/path/to/reference_thesis_or_template.docx"
export SWUN_CSL="/path/to/china-national-standard-gb-t-7714-2015-numeric.csl"
export SWUN_BIB="/path/to/references.bib"

# Regenerate the bundled slim template from a full reference thesis
python3 /Users/bit/.claude/skills/swun-thesis-docx-banshi1/scripts/make_slim_template.py \
  --input /path/to/full_reference.docx \
  --output /Users/bit/.claude/skills/swun-thesis-docx-banshi1/assets/swun_banshi1_reference.docx
```
...
2. 确认内置瘦身模板存在（skill 自包含，默认无需外部模板）：
   ```bash
   ls /Users/bit/.claude/skills/swun-thesis-docx-banshi1/assets/swun_banshi1_reference.docx
   ```

3. （可选）配置环境变量覆盖默认路径：
   ```bash
   export SWUN_REFERENCE_DOCX="/path/to/reference_thesis.docx"
