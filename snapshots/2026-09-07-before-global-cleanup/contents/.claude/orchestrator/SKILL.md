...
|------|-----------|----------|-------|
| **Team + nested waves (Pattern E)** | TeamCreate + subagent waves per teammate — up to ~50 agents total | 2+ independent streams — THE DEFAULT | sonnet (default) |
| **Subagent wave** | Task tool, parallel calls | 2+ actions, single domain, no coordination, no context pressure | sonnet (default) |
...
### Model selection
- Default policy: `sonnet` for every subagent, teammate, and wave. Opus is reserved for main session only.
...
3. Never reduce parallelism or tier for any reason. Subagents always use sonnet; opus is reserved for main session only.
