...
1. Generate an overlap + discovery cleanup report from installed skills:
   - `python3 skills/skill-consolidator/scripts/scan_installed_skills.py --out-dir tmp/skill-consolidator --include-system`
2. Review `tmp/skill-consolidator/collision_report.md`, `canonical_map.json`, and `quarantine_report.md`.
3. Dry-run cleanup:
   - `python3 skills/skill-consolidator/scripts/apply_discovery_cleanup.py --inventory tmp/skill-consolidator/inventory.json`
4. Execute cleanup only after review:
   - `python3 skills/skill-consolidator/scripts/apply_discovery_cleanup.py --inventory tmp/skill-consolidator/inventory.json --execute`
5. Build the canonical result as a first-party skill in `skills/<new-skill-name>/` only after discovery pollution is under control.
...
- `canonical_map.json` (one winner per discoverable skill name)
- `collision_report.md` (same-name conflicts + chosen winner)
- `quarantine_report.md` (non-discoverable entries and why)
...
### 2a) Discovery cleanup before consolidation

If the inventory shows docs copies, backup copies, template/example skills, or shadowed duplicates, disable them before deeper consolidation work:
...
- Execute:
  - `python3 skills/skill-consolidator/scripts/apply_discovery_cleanup.py --inventory tmp/skill-consolidator/inventory.json --execute`
- Restore if needed:
  - `python3 skills/skill-consolidator/scripts/restore_discovery_cleanup.py --manifest tmp/skill-consolidator/cleanup_manifest.json`
