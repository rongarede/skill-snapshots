...
| Skill | 描述 | 触发词 |
|-------|------|--------|
| collaborating-with-codex | Codex CLI 协作后端 | /codex、调用 codex |
| collaborating-with-gemini | Gemini CLI 协作后端 | /gemini、调用 gemini |
| collaborating-with-kimi | Kimi CLI 协作后端 | /kimi、调用 kimi |
...
| Skill | 描述 | 触发词 |
|-------|------|--------|
| skill-catalog | 汇总所有 Skills 目录 + 迭代复盘 + 编写准则，支持 GitHub 更新检查（本 skill） | /skills、/skill复盘、/迭代清单 |
...
| 统一调用外部 AI | collaborating-with-codex / gemini / kimi（直接调用） |
...
| 复盘技能使用情况 | skill-catalog (`/skill复盘`) |
| 编写新 Skill | skill-catalog（内含编写准则） |
...
> 原 `skill-retrospective` 合并至此。触发词：`/skill复盘`、`/迭代清单`、「帮我整理一下今天的 skill 迭代」

### 工作流程

1. **回顾对话** — 扫描当前对话，识别调用了哪些 skill、产出了什么、用户反馈
2. **提取负反馈** — 直接否定、修正意见、质疑、补充要求，记录 skill 名 + 用户原话 + 问题本质
3. **分析迭代方向**：
   - **更新现有 Skill**：问题指向具体步骤/规则，增加检查点可解决
   - **新建 Skill**：完整新工作流、现有 skill 不覆盖、可复用
   - **不需改动**：一次性特殊情况
4. **生成迭代清单** — 输出格式见下方模板
5. **自动快照** — 更新完成后自动执行 `skill-snapshot save`
...
- 必须有 shebang (`#!/bin/bash` 或 `#!/usr/bin/env python3`)
- 必须有执行权限 (`chmod +x`)
- Bash 使用 `set -e` 遇错即停
