...
- Before creating files, run `python scripts/resolve_output_paths.py --cwd "$(pwd)" --topic "[topic]"`. Use its `deliverable_dir` and `evidence_dir` fields rather than hand-writing paths.
- When the current workspace is inside an Obsidian vault, save the reader-facing research deliverables under `300_Resources/Res_调研/[Topic]_Research_[YYYYMMDD]/`. Discover the vault root by walking upward for the PARA directories; do not hard-code a user-specific vault path.
- Write the Markdown report, HTML report, PDF, and a concise `README.md` in that resource folder. The Markdown report is the source of truth for the deliverable.
- Keep machine-oriented, regenerable run state outside the vault: `sources.jsonl`, `evidence.jsonl`, `claims.jsonl`, `run_manifest.json`, temporary render files, and continuation state. Link or mention the external evidence-run location in the report's Methodology Appendix when it is useful for reproducibility.
- When no Obsidian vault is in scope, save all deliverables to `~/Documents/[Topic]_Research_[YYYYMMDD]/`, as before.
- Do not create a new PARA category for a report. Reuse `300_Resources/Res_调研`, and create only the dated topic folder inside it.
